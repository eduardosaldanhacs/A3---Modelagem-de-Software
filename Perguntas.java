...
public class Perguntas {
    ...
    public void exibirPerguntaEAlternativas() {
        System.out.println("\n" + pergunta);
        for (int i = 0; i < alternativas.length; i++) {
    ...
    public void processarExplicacao(Scanner scanner) {
        if (explicacao != null) {
    ...
    public void processarDica(Scanner scanner, int moedas, boolean PodeUsarDica) {
        if (usouDica) {
            System.out.println("\nVocê não pode utilizar uma dica em duas rodadas consecutivas!");
            System.out.println("Por favor, espere pela próxima rodada.");
            return;
        }
        ...
        while (true) {
            ...
            System.out.println("2- Remover 2 alternativas (custo: 10 moedas).");
            System.out.println("3- Remover 3 alternativas (custo: 15 moedas).");
            System.out.println("Você tem " + moedas + " moedas.");
            System.out.println("Digite 1, 2 ou 3 para escolher a dica:");

            String escolha = lerEntrada(scanner);

            try {
                int numDica = Integer.parseInt(escolha);
                int custo;
                switch (numDica) {
                    case 1 -> custo = 5;
                    case 2 -> custo = 10;
                    ...
                    default -> {
                        ...
                        continue; // Agora pode usar continue aqui
                    }
                }

                if (Dica(numDica, moedas, PodeUsarDica)) {
                    moedas -= custo;
                    System.out.println("Você ficou com " + moedas + " moedas.");
                    this.usouDica = true;
                    break;
                }
    ...
    private boolean processarResposta(Scanner scanner) {
        System.out.println("Digite o número da resposta (1-" + alternativas.length + "):");
        String respostaUsuario = lerEntrada(scanner);
        try {
            return Integer.parseInt(respostaUsuario) == respostaCorreta;
        } catch (NumberFormatException e) {
            return false;
        }
    }
    ...
    public boolean fazerPergunta2(Scanner scanner, boolean PodeUsarDica, int moedas) {
